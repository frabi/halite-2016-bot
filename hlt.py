...
class GameMap:
    ...
    def neighbors_xy(self, x, y):
        combos = ((0, -1), (1, 0), (0, 1), (-1, 0))   # NORTH, EAST, SOUTH, WEST, STILL ... matches indices provided by enumerate(game_map.neighbors(square))

        return ( ((x + dx) % self.width, (y + dy) % self.height) for dx, dy in combos if dx or dy)

    def neighbors_ninebox(self, x, y, a, b):

        combos = ((0, -1), (1, 0), (0, 1), (-1, 0))
        ans = []
        for dx, dy in combos:
            ans_x, ans_y = x + dx, y + dy
            ans_a, ans_b = a, b
            # all illegal moves
            illegal = (ans_x < 0 and ans_a == 0) or (ans_y < 0 and ans_b == 0) \
                or (ans_x > self.width - 1 and ans_a == 2) or (ans_y > self.height - 1 and ans_b == 2)
            if illegal:
                continue
            elif ans_x < 0:
                ans_x += self.width
                ...
                ans_y += self.height
                ans_b -= 1
            elif ans_x > self.width - 1:
                ans_x -= self.width
                ans_a += 1
            elif ans_y > self.height - 1:
                ans_y -= self.height
                ans_b += 1
            ans.append((ans_x, ans_y, ans_a, ans_b))
        return ans
    ...
    def get_distance_2(self, sq1, sq2):
        "Returns Manhattan distance between two squares."
        dx = min(abs(sq1.x - sq2.x), sq1.x + self.width - sq2.x, sq2.x + self.width - sq1.x)
        dy = min(abs(sq1.y - sq2.y), sq1.y + self.height - sq2.y, sq2.y + self.height - sq1.y)
        return dx, dy, dx + dy
